...
def restyle(svg: str) -> str:
    ...
    svg = svg.replace("fill:#f7fbff", "fill:#58636d")
    svg = svg.replace("fill:#afc8e3", "fill:#b7cce0")
    svg = re.sub(
        r"fill:#58a6ff",
        "fill:#58a6ff",
        svg,
    )
    svg = svg.replace("fill:#f8fafc", "fill:#58636d")
    svg = svg.replace("fill:#dce9f6", "fill:#58636d")
    svg = svg.replace("fill:#dbe6f2", "fill:#b7cce0")
    return svg
...
{optical_material(width, height)}
  ...
  <text x="916" y="108" text-anchor="middle" fill="#58636D" font-family="Consolas,Menlo,monospace" font-size="13" font-weight="800">SB</text>
</svg>
'''
