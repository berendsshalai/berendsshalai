#!/usr/bin/env python3
"""Validate the generated cinematic Emerald Observatory statistics assets."""
...
GIF_NAMES = ("ascii-stats-gallery-color-transparent.gif",)
PNG_NAMES = ("ascii-stats-gallery-color-transparent-static.png",)
...
def visual_metrics(image: Image.Image) -> tuple[bool, float, int]:
    rgba = image.convert("RGBA")
    alpha = rgba.getchannel("A")
    corners = ((0, 0), (rgba.width - 1, 0), (0, rgba.height - 1), (rgba.width - 1, rgba.height - 1))
    opaque_corners = all(alpha.getpixel(point) == 255 for point in corners)
    histogram = alpha.histogram()
    opaque_ratio = sum(histogram[250:]) / (rgba.width * rgba.height)
    colorful = 0
    ...
    for red, green, blue, opacity in pixels:
        if opacity > 0 and max(red, green, blue) - min(red, green, blue) >= 55:
            colorful += 1
    return opaque_corners, opaque_ratio, colorful
...
def validate_gif(name: str) -> dict[str, float | int | str]:
    ...
    with Image.open(path) as image:
        ...
        for index in range(frames):
            ...
            if index == 0:
                ...
                opaque_corners, opaque_ratio, colorful = visual_metrics(frame)
                if not opaque_corners or opaque_ratio < 0.99:
                    fail(f"{name} is not an opaque cinematic plate")
                if colorful < 250:
    ...
    return {"name": name, "bytes": path.stat().st_size, "frames": frames, "duration_seconds": duration / 1000, "seam_rms": round(seam_rms, 2), "opaque_ratio": round(opaque_ratio, 3), "colorful_samples": colorful}
...
def validate_png(name: str) -> dict[str, int | float | str]:
    ...
    with Image.open(path) as image:
        ...
        if image.size != EXPECTED_SIZE:
            fail(f"{name} dimensions are {image.size}")
        opaque_corners, opaque_ratio, colorful = visual_metrics(image)
        if not opaque_corners or opaque_ratio < 0.99:
            fail(f"{name} is not an opaque cinematic plate")
        if colorful < 250:
            fail(f"{name} does not retain enough saturated accent pixels")
    return {"name": name, "bytes": path.stat().st_size, "opaque_ratio": round(opaque_ratio, 3), "colorful_samples": colorful}
...
def main() -> None:
    ...
    print("PASS: cinematic Emerald Observatory statistics assets validated.")
